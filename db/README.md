...
The `sale_items` table stores the **price at time of sale** (`soldPrice`, `soldMakingCharge`, `soldWastage`, etc.) — not a reference to the current item price. This ensures historical bills remain accurate even if item data is later updated.
...
The `PrismaClient` instance is exported as a module-level singleton from `prismaClient.js`. This prevents connection pool exhaustion in serverless environments (Vercel), where each function invocation could otherwise spawn a new connection.
